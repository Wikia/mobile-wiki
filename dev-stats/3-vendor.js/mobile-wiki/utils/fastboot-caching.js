define('mobile-wiki/utils/fastboot-caching', ['exports'], function (exports) {
	'use strict';
	...
	exports.setResponseCaching = setResponseCaching;
	/**
  ...
  */
	...
	var CachingPolicy = exports.CachingPolicy = {
		Private: 'private',
		Public: 'public'
	},

	// Caching expire intervals
	CachingInterval = exports.CachingInterval = {
		// 30 days in seconds
		long: 2.592e+6,
		// 24h in seconds
		standard: 8.64e+4,
  ...
  * @returns {void}
  */
	function disableCache(fastboot) {
  ...
  * @returns {void}
  */
	function setResponseCaching(fastboot, cachingSettings) {
		if (!fastboot.get('isFastBoot')) {
			return;
		}
		...
		if (cachingSettings && cachingSettings.enabled) {
			...
			if (cachingSettings.browserTTL > 0) {
				fastboot.get('response.headers').set('X-Pass-Cache-Control', cachingSettings.cachingPolicy + ', max-age=' + cachingSettings.browserTTL);
			}
		}
	}
});
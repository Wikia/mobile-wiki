define('mobile-wiki/tests/unit/models/wikia-nav-test', ['ember-qunit', 'sinon', 'mobile-wiki/models/wikia-nav'], function (_emberQunit, _sinon, _wikiaNav) {
	'use strict';

	(0, _emberQunit.moduleFor)('model:wikia-nav', 'Unit | Model | wikia nav', {
		unit: true
	});
	...
	(0, _emberQunit.test)('test zero state with values from api', function (assert) {
		var cases = [{
			mock: {
				hubsLinks: [],
				localLinks: [],
		...
		cases.forEach(function (testCase) {
			var nav = _wikiaNav.default.create(testCase.mock);

			assert.deepEqual(nav.get('items'), testCase.expected, testCase.message);
		});
	});

	(0, _emberQunit.test)('test local sub nav transitions', function (assert) {
		var cases = [{
			mock: {
				hubsLinks: hubsLinksMock,
				localLinks: [{
		...
		cases.forEach(function (testCase) {
			...
			testCase.path.forEach(function (i) {
				nav.goToSubNav(i);
			});
			assert.deepEqual(nav.get('items'), testCase.expected, testCase.message);
		});
	...
	(0, _emberQunit.test)('Header value', function (assert) {
		var cases = [{
			mock: {
				hubsLinks: hubsLinksMock,
				localLinks: [{
	...
	(0, _emberQunit.test)('Parent value', function (assert) {
		var cases = [{
			mock: {
				hubsLinks: hubsLinksMock,
				localLinks: [{

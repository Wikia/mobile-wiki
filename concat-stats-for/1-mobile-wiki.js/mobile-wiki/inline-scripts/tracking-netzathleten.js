define('mobile-wiki/inline-scripts/tracking-netzathleten', [], function () {
	'use strict';
	...
	(function () {
		...
		if (!M.getFromShoebox('runtimeConfig.noExternals') && !M.getFromShoebox('serverError') && config.enabled && config.url && !isLoggedIn && window.Wikia) {
			allowedCountries = window.Wikia.InstantGlobals.wgAdDriverNetzAthletenCountries || [];

			if (allowedCountries.indexOf(geo.country) !== -1 || allowedCountries.indexOf('XX') !== -1) {

define('mobile-wiki/inline-scripts/tracking-nielsen', [], function () {
	'use strict';
	...
	(function (M) {
		...
		window.trackNielsenPageView = function () {
			if (M.getFromShoebox('runtimeConfig.noExternals') || M.getFromShoebox('serverError') || !config.enabled) {
				return;
			}
			...
			M.loadScript('https://secure-dcr.imrworldwide.com/novms/js/2/ggcmb500.js?' + Date.now(), true, sendNielsenPageView);
		};

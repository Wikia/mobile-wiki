...
export default function getCuratedMainPageThumbs(data) {
	...
	if (mainPageData && mainPageData.featuredContent) {
		mainPageData.featuredContent.forEach(function(item){
	...
	if (mainPageData && mainPageData.curatedContent) {
		mainPageData.curatedContent.forEach(function(item){
			if (item.image_url) {
				item.thumb_url = getImageThumb(
					item.image_url,
					200,
...
function getImageThumb(image_url, width, height, mode, imageCrop) {
	const options = {
		width
	};
	...
	return Vignette.getThumbURL(image_url, options);
}

	...
	private getOnPreResponseHandler(isDevbox: boolean) {
		return (request: hapi.Request, next: Function): void => {
			...
			if (!isDevbox && response.variety === 'file') {
				response.header('Cache-control', 's-maxage=' + Caching.Interval.long);
				response.header('X-Pass-Cache-Control', Caching.policyString(Caching.Policy.Public) + ', max-age=' +
					Caching.Interval.long);
			}

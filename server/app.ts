	...
	private getOnPreResponseHandler(isDevbox: boolean) {
		return (request: hapi.Request, next: Function): void => {
			...
			if (!isDevbox && response.variety === 'file') {
				response.header('Cache-control', 's-maxage=' + utils.Caching.interval.long);
				response.header('X-Pass-Cache-Control', utils.Caching.policy.public +', max-age=' + utils.Caching.interval.long);
			}

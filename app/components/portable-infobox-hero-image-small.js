...
export default Ember.Component.extend(
	...
	{
		isLoading: true,
		thumbnailUrl: Ember.computed('heroImage', 'viewportDimensions.width', function () {
			const heroImage = this.get('heroImage'),
				maxWidth = Math.round(this.get('viewportDimensions.width') * 0.7),
				heroImageHelper = new HeroImage(heroImage, maxWidth);

			return heroImageHelper.thumbnailUrl;
		}),
		...
		init() {
			...
			this.load(this.get('thumbnailUrl')).then(() => {
